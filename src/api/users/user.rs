...
use crate::api::users::token::{verify_token, Error};
...
pub async fn current_user(req: HttpRequest, pool: Data<PgPool>) -> HttpResponse {
    match check_by_token(req, &pool).await {
        Ok((ok, token)) => return HttpResponse::Ok().json(json!({"user": {
            "email": ok.email,
...
async fn check_by_token(req: HttpRequest, pool: &PgPool) -> Result<(User, String), Error> {
    ...
    match verify_token(&token) {
        Ok(token_data) => {
            ...
            let user = sqlx::query_as!(User, "SELECT id, email, username FROM accounts WHERE id = $1", user_id)
                .fetch_one(pool)
                .await;

            if user.is_err() {
                return Err(Error::AccountDoesNotExist("Пользователь не существует".to_string()));
            }
